...
with open("context.yaml", "r") as spec_file:
...
json_data = json.loads(open("context.json", "r").read())
Context = create_python_object(response_schema["items"], "Context", json_data)

for attr, value in vars(Context).items():
    print(f"{attr}: {value}")
